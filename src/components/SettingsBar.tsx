...
export default function SettingsBar({ policy, onPolicy, capacity, onCapacity, onReset }: Props) {
  ...
  const applyCap = () => {
    const n = parseInt(capField, 10)
    if (!Number.isNaN(n) && n > 0) onCapacity(n)
  }
  const Btn = ({ name, value }: { name: string; value: Policy }) => (
    <button
      ...
      className={clsx(
        ...
        policy === value ? 'bg-brand-600 text-white shadow' : 'bg-neutral-800 hover:bg-neutral-700'
      )}
    >
      {name}
    </button>
    ...
    <div className="flex items-center gap-3 p-3 border-b border-neutral-800 bg-neutral-900/60 backdrop-blur">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <input
          ...
          className="w-20 bg-neutral-800 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 ring-brand-600"
          type="number"
          min={1}
        />
        <button onClick={onReset} className="px-3 py-2 rounded-xl text-sm bg-neutral-800 hover:bg-neutral-700">Reset</button>
      </div>
    </div>
  )
}