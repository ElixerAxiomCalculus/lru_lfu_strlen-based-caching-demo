...
type Props = { policy: Policy; entries: CacheEntry<string, string>[]; stats: { hits: number; misses: number; puts: number; evictions: number; policy: Policy }; snapshot: unknown; lastAction: string }
          ...
          <MotionConfig transition={{ type: 'spring', stiffness: 400, damping: 35 }}>
            <div className="flex flex-col gap-2">
              <AnimatePresence initial={false}>
                {entries.map((e) => (
                  <motion.div key={String(e.key)} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} className="rounded-2xl p-3 bg-[var(--card-bg)] border border-neutral-800 flex items-center justify-between">
