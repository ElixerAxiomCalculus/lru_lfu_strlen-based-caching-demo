...
export class LRUCache<K, V> implements ICache<K, V> {
  ...
  put(key: K, value: V) {
    ...
    this.putsCount++
    if (n) { n.value = value; this.moveToHead(n); return }
...
export class LFUHeapCache<K, V> implements ICache<K, V> {
  ...
  constructor(capacity: number) {
    ...
    this.heap = new BinaryHeap((a, b) => a.freq !== b.freq ? a.freq - b.freq : a.time - b.time)
  }
  capacity() { return this.cap }
  size() { return this.map.size }
  has(key: K) { return this.map.has(key) }
  ...
  put(key: K, value: V) {
    ...
    if (existing) { existing.value = value; existing.freq += 1; existing.time = ++this.t; this.heap.update(existing); return }
    if (this.map.size >= this.cap) {
...
export class LengthTimeHeapCache<K, V extends string> implements ICache<K, V> {
  ...
  put(key: K, value: V) {
    this.putsCount++
    const existing = this.map.get(key)
    if (existing) { existing.value = value; existing.length = String(value).length; existing.time = ++this.t; this.heap.update(existing); return }
    if (this.map.size >= this.cap) {
      const root = this.heap.pop()
      if (root) { this.map.delete(root.key); this.evicts++ }
    }
