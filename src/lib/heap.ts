export class BinaryHeap<T> {
  ...
  push(x: T) { this.data.push(x); this.indexMap.set(x, this.data.length - 1); this.bubbleUp(this.data.length - 1) }
  pop(): T | undefined {
    if (this.data.length === 0) return undefined
    const root = this.data[0]
    const last = this.data.pop() as T
    this.indexMap.delete(root)
    if (this.data.length > 0) { this.data[0] = last; this.indexMap.set(last, 0); this.bubbleDown(0) }
    return root
  }
  update(x: T) {
    const i = this.indexMap.get(x)
    if (i === undefined) return
    if (i > 0 && this.cmp(this.data[i], this.data[this.parent(i)]) < 0) this.bubbleUp(i)
  ...
  private swap(i: number, j: number) {
    ...
    this.data[i] = vj; this.data[j] = vi
    this.indexMap.set(vj, i); this.indexMap.set(vi, j)
  }
  private bubbleUp(i: number) {
    while (i > 0) {
      const p = this.parent(i)
      if (this.cmp(this.data[i], this.data[p]) < 0) { this.swap(i, p); i = p } else break
    }
  }
  private bubbleDown(i: number) {
    ...
    while (true) {
      ...
      if (m !== i) { this.swap(i, m); i = m } else break
    }
  }
}
